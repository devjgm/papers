...
standard *Civil-Time Library* ([D0215R0]). The current paper is proposing a
...
[D0215R0]. This library is implementable using only C++98, and requires no
language extensions.
...
described in the proposed Civil-Time Library ([D0215R0]). The Time-Zone Library
...
```cpp
#include <chrono>
#include "civil.h"  // See proposal [D0215R0]
...
Time-zone data for the named zone is loaded on the first request for that data,
and the data is cached for the lifetime of the process. Subsequent calls to
`load_time_zone()` will use the cached data, avoiding the need to load the data
again.
